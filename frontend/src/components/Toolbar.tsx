...
export function Toolbar({ busy, conversionMode, viewMode, visionStatus, onModeChange, onViewChange, onOpenSettings, onUpload, onAnalyze, onExport, onAddShape, onDelete, onCopy, onLayer }: Props) {
  return <header className="toolbar">
    ...
    <select className="mode-select" value={conversionMode} onChange={(event) => onModeChange(event.target.value as Props['conversionMode'])} disabled={busy} aria-label="转换模式"><option value="fast">快速</option><option value="standard">标准</option><option value="high_quality">高精度</option><option value="maximum">最多三轮纠偏</option></select>
