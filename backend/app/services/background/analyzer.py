...
def analyze_background(image: np.ndarray, bbox: list[float]) -> dict[str, Any]:
    ...
    if variance < 180 and max(horizontal_gradient, vertical_gradient) < 12:
        category = "solid"
    elif max(horizontal_gradient, vertical_gradient) >= 12 and texture < 500:
        category = "gradient"
    elif texture < 1800:
        category = "texture"
    else:
        category = "complex"
    return {"category": category, "medianBgr": median.tolist(), "leftColor": left.tolist(), "rightColor": right.tolist(), "topColor": top.tolist(), "bottomColor": bottom.tolist(), "variance": variance, "texture": texture, "bbox": [x1, y1, x2, y2]}
